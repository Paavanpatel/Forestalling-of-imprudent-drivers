...
def sound_alarm(path):
    ...
    #subprocess.Popen(['mpg123','-o l',path]).wait()
...
def mouth_opening_ratio(mouth):
    ...
    D =euclidean_dist(mouth[12], mouth[16])
...
def drowsiness():
    ...
    #ap.add_argument("-c", "--cascade", required=True, default= "haarcascade_frontalface_default.xml",
    ...
    MOUTH_OP_THRESH = 0.15
    MOUTH_OP_CONSEC_FRAMES = 36
    ...
    while True:
        ...
        for (x, y, w, h) in rects:
            ...
            mMOR = mouth_opening_ratio(mouth)
            ...
            if ((ear < EYE_AR_THRESH)  or (mMOR > MOUTH_OP_THRESH)):
                COUNTER += 1

                # if the eyes were closed for a sufficient number of
                # frames, then sound the alarm
                if ((COUNTER >= EYE_AR_CONSEC_FRAMES) or (COUNTER >= MOUTH_OP_CONSEC_FRAMES)):
                    ...
                    cv2.putText(frame, "DROWSINESS ALERT!", (30, 300),
            ...
            cv2.putText(frame, "EAR: {:.3f}".format(ear), (300, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.putText(frame, "MOR: {:.2f}".format(mMOR), (30, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
